...
(defn update-state [state opcode params modes rel-base]
    (cond 
        ...
        (= opcode 3) (assoc state 
                            ...
                            (do (print "Choose \"1\" for Part 1 or \"2\" for Part 2: ") (flush) (read-string (read-line))))
        (= opcode 4) (do (println (parameter-value state (first params) (first modes) rel-base)) state)
...
(def res (subvec
    (run-intcode (allocate-additional-mem initial-state (* 100 (count initial-state))))
    0
    (count initial-state)))
